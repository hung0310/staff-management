...
import { Approve_Leave_Request, List_Leave_Request_Manager, Reject_Leave_Request } from '../../../../../../../apis/staffAPI';
...
const LeaveRequest = () => {
    ...
    const fetchDataRequest = async () => {
        try {
            const result = await List_Leave_Request_Manager(currentPage);
        ...
        if(isSubmitApprove) {
            try {
                const fetchData = async () => {
                    const result = await Approve_Leave_Request({leave_request_id: idRequest});
                    if(result.status === 200) {
    ...
    useEffect(() => {
        if(isSubmitReject) {
            try {
                const fetchData = async () => {
                    const result = await Reject_Leave_Request({leave_request_id: idRequest});
                    if(result.status === 200) {
